...
def execute_and_plot(model_name, detector_method, self_supervised, all_curves, current_params, run_over, num_of_rep=1):
    ...
    ser = get_ser_data(trainer, run_over=run_over, num_of_rep=num_of_rep, method_name=method_name + '_' + current_params)
...
if __name__ == '__main__':
    ...
    num_of_test_rep = 10  # number of test blocks multiplications
    ...
    models_list = ['Transformer', 'ViterbiNet', 'SionnaPlus', 'ClassicViterbi']
    ...
    for snr in range(snr_start, snr_end+1):
        ...
        gc.collect()
        print(f'SNR = {snr}, block_length = {block_length}, ECC_N_Symbols = {n_symbol}')
        ...
        for model in models_list:
            if model == 'ClassicViterbi':
                execute_and_plot(model, 'Statistical', False, all_curves, current_params, run_over, 1)  # Classic Viterbi Alg with Perfect-CSI
            else:
                execute_and_plot(model, detector_method, self_supervised, all_curves, current_params, run_over, num_of_test_rep)
