...
from Code.ecc.polynomials_manipulation import gf_poly_eval, gf_pow, gf_poly_scale, gf_poly_add, gf_inverse, \
    gf_mul, \
...
def rs_correct_errata(msg_in, synd, err_pos):  # err_pos is a list of the positions of the errors/erasures/errata
    ...
    for i, Xi in enumerate(X):
        ...
        # Compute the formal derivative of the error locator polynomial (see Blahut, Algebraic Code for data transmission, pp 196-197).
        # the formal derivative of the errata locator is used as the denominator of the Forney Algorithm, which simply says that the ith error value is given by error_evaluator(gf_inverse(Xi)) / error_locator_derivative(gf_inverse(Xi)). See Blahut, Algebraic Code for data transmission, pp 196-197.
        err_loc_prime_tmp = []
...
def rs_find_error_locator(synd, nsym, erase_loc=None, erase_count=0):
    ...
    for i in range(0,
        ...
        # This optimization is actually described in several figures of the book "Algebraic Code for data transmission", Blahut, Richard E., 2003, Cambridge university press.
        delta = synd[K]
